...
def menu(): #menu
    ...
    def search(): #search for the word
        ...
        URL = 'https://www.thesaurus.com/browse/'+word #generate the url by putting the word in
        page = requests.get(URL) #request the url

        soup = BeautifulSoup(page.content, 'html.parser') #get the html from requested webpage

        URL1 = 'https://www.thesaurus.com/browse/'+word
        page = requests.get(URL1)
        tree = html.fromstring(page.content)
        
        for x  in range(1,15):
            syn = tree.xpath('//*[@id="meanings"]/div[2]/ul/li['+str(x)+']/a/text()')
            if(len(syn)>0):
                synonym.append(tree.xpath('//*[@id="meanings"]/div[2]/ul/li['+str(x)+']/a/text()')[0])
            ant = tree.xpath('//*[@id="antonyms"]/div[2]/ul/li['+str(x)+']/a/text()')
            if(len(ant)>0):    
                antonym.append(tree.xpath('//*[@id="antonyms"]/div[2]/ul/li['+str(x)+']/a/text()')[0])
        ...
        #synonym = tree.xpath('//*[@id="meanings"]/div[2]/ul/li[1]/a/text()')        
